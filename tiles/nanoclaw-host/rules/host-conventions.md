...
There are two agents improving this system: the container agent (the container agent) and the host agent (you). Both make useful updates asynchronously. Never assume you have the latest version of anything. Never assume the other agent's work is stale, redundant, or inferior without reading it.
...
You own the full stack: source code, tiles, scripts, deployment, NAS, containers. If you find a problem anywhere — fix it. Don't say "that's the container agent's skill to fix" or "the container agent should handle that." If you can fix it from here, fix it from here.
