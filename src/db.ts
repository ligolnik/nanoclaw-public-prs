...
export function resurrectZombieTasks(): string[] {
  ...
  // "interrupted dispatches" — they're tasks whose schedule already
  // passed, the scheduler stamped them completed, and the agent
  // forgot to populate last_run. Pulling them back active makes
  // them re-fire the moment the orchestrator next loops, sending
  // a stale prompt to a chat long after the user's intent has
  // expired.
  //
  // Failure mode without this gate: a once-task scheduled for
  // "send X to group on Apr 24" gets resurrected on every restart
  // and re-fires through Apr 25, 26, 27... Each restart adds
  // another stale send.
  //
  // The `next_run > now` clause keeps the original intent (revive
